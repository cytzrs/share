...
def is_in_trading_hours() -> Tuple[bool, Optional[str]]:
    """
    检查当前是否在A股交易时段内
    
    Returns:
        Tuple[bool, Optional[str]]: (是否在交易时段内, 跳过原因)
    """
    now_dt = datetime.now()
...
class TaskExecutor:
    ...
    async def _execute_single_agent(self, agent: ModelAgentModel) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查交易时段
            in_trading_hours, skip_reason = is_in_trading_hours()
            if not in_trading_hours:
                return {
                    ...
                    "completed_at": now().isoformat(),
                    "duration_ms": 0,
                    "error_message": f"非交易时段: {skip_reason}",
                }
